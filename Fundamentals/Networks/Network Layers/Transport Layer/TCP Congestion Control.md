...
| **TCP Cubic**    | Uses a cubic function for `cwnd` growth. Increases `cwnd` more aggressively after long RTTs but slows near saturation.           | Superior performance in high-speed, long-delay networks.- Efficiently utilizes bandwidth.- Default algorithm in Linux | Can be aggressive in mixed network environments.- May cause unfairness to delay-based flows                       |
